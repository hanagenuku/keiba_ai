...
def job(a):
    ...
    ci = np.clip(iso.predict(pi), 1e-6, 1 - 1e-6)
    return w, seed, pv, cv, int(m.best_iteration), ci
...
if __name__ == '__main__':
    ...
    for w in WINS:
        ...
        out[w] = dict(prob=np.mean([r[2] for r in rs], axis=0),
                      ...
                      cal_in=np.mean([r[5] for r in rs], axis=0),
                      trees=[r[4] for r in rs])
        print(f'{w}: 木 {out[w]["trees"]}  n={len(out[w]["prob"]):,}', flush=True)
    with open(f'{BASE}/cut_pred.pkl', 'wb') as f:
