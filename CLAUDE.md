...
| **base_margin を人気順位→実オッズに** | **+0.008〜0.020**（2026-08-06実測） | **データ待ち**。`race_predictions.tansho_odds`（朝オッズ）が現在**603レース**、月およそ300で N=2,000 まで約5ヶ月 |

**D-1 で残っているのは実オッズアンカーだけ**で、それは着手できない。放置しても
毎週勝手に溜まるので、半年後に自動的に着手可能になる、という位置づけ。
