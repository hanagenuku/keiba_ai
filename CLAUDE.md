...
| 2026-08-08 | Plackett-Luceレーティング（着順全体から馬の潜在能力θを推定し、直接対戦していない馬同士も比較可能にする） | **本番パイプラインで +0.0003（誤差）**。⚠ 一度「+0.037 / 回収率115%」と報告したが、検証スクリプトのタイブレークが着順を漏らすリークだった（SQLが`ORDER BY place`、θ同値の馬で順位=着順）。**θ自体は正しく、θを対象にした検査は全部通っていた**。インフラは残置（週次でθ更新）。学習データを増やす・騎手情報を厚くする・馬体重の欠損を埋める、の3案も同時にオラクル法で否定済み |
...
### 最終更新: 2026-08-08③（Plackett-Luceレーティング導入 → 効果 +0.0003。🔴 一度リークで +0.037 と誤報告した）

#### ⚠ 最初にこれを読むこと
このセッションで一度「**AUC +0.037 / 回収率115%**」と報告したが、
**検証コードのリークによる誤りだった**。正しくは **+0.0003（誤差）**。
...
#### 🔴 結果：+0.0003（誤差）。予測精度は上がらなかった
本番パイプラインで学習データを作り直して測った（val 2026-07-01〜08-02、3シード）:
...
#### 現在の扱い（ユーザー判断: インフラは残す）
- **本番モデルには入れていない**（`xgb_feature_cols.json` は129特徴量のまま）。
  +0.0003 のために再学習する価値がないため
- **θは週次で更新され続ける**（`sunday-results.yml`）。出走回数が増えれば
  推定が細かくなるので、数ヶ月後に測り直す価値はある
