...
### 最終更新: 2026-08-06②（システム全体の段階別監査：ペース特徴量のリークを発見・修正＋死んだコード1,862行を削除）
...
### ⚠ 削除を見送ったもの（ノートブックが参照）
`src/betting/dual_model.py`(157) と `src/betting/rating_calibration.py`(269) は
本番から完全に孤立している（`make_bets_v2` は `**_kwargs` で引数を飲み込むだけで
`feat_df` 経路自体が存在しない）が、`KEIBA_XGB_retrain_v5.ipynb` の
強制アップデートセルが取得しに行くため、消すとノート実行時に404が出る。
**削除するならノート側のファイルリスト修正が必須**。
同様に `data/xgb_ranking_ndcg.pkl`(287KB) も本番未使用のまま残置。
