...
8. 迷ったら**DESIGN.mdの「やってはいけないこと」表**を確認する。書いていなければ
   実装せずまず確認・相談する
...
### 最終更新: 2026-07-28①（買い目フィルタを大規模検証で否定・無効化）

---

## 🚩 次セッションはここから読む（2026-07-28終了時点のサマリ）

### 🔴 最重要：買い目フィルタは大規模検証で否定され、無効化された
...
### いま何が動いているか
- 単勝・複勝は**従来ロジック**（RL上位3頭から1点／RL上位5頭から2点）に戻っている
- マトリクスの**測定機能**は残っている（stats.json / divergence_weekly.json）。
  止めたのは買い目への適用のみ
- ユーザーは**エア馬券で運用中**（実損は出ていない）

### ⚠ 正直な現状認識
**現時点で利益の出る単勝戦略は存在しない。** 従来ロジックも本番データで94.8%、
検証でneutralが61.9%と、どちらも控除率を超えられていない。
「フィルタが効かない」だけでなく「まだエッジが見つかっていない」状態。

### 次の週末（2026-08-01・08-02）で確認すること
1. **refreshの自動実行が初めて動く**（cron 08:00 JST 土日）
2. **prediction_snapshots に initial と refresh の両方が入るか**
3. **bet_simulation 2,160行が一括決済される** → 馬連・三連複・ワイドの有効性が初めて判定可能に
4. オッズ異常の警告ログが出ないか（出なければ正常）
