...
10. 迷ったら**DESIGN.mdの「やってはいけないこと」表**を確認する。書いていなければ
    実装せずまず確認・相談する
...
| 2026-08-25 | D-2: 残差学習の base_margin を人気順位のZipf近似→**実オッズ**にする（2026-08-06 に +0.008〜0.020 AUC と実測され、残っている中で桁が一つ大きい唯一の案だった） | **基準未達で不採用**。netkeiba から実オッズを53,916頭ぶん取得し、3,940レース・53,715頭で測定。**+0.0043**で事前登録の2基準とも未達（①全窓プラス 2/3窓 ②平均+0.01以上）。🔑 内訳が決定的で、**確定オッズなら +0.0254 なのに朝のオッズ相当に落とすと +0.0042 しか残らない**＝実オッズの価値の**83%は「発走直前まで待てること」由来**で、本番が見る朝の時点では消えている（2026-08-03「オッズ観測を細かくしても上限94.5%」と同じ構図）。⚠ しかもこの +0.0043 は**甘めに出ている**（ドリフトの元にした race_predictions が上書きされ、08-17のrefresh 3回化以降は朝でなく14:00のオッズが残るため）。✅ 取得した実オッズは artifact に残っており別の問いには使える |
