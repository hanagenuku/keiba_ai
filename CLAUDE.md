...
### 最終更新: 2026-07-27⑧（単勝オッズの下限バリデーションを実装）

---

### 2026-07-27⑧：単勝オッズ1.0倍未満を弾くバリデーションを実装

#### 背景
⑦で「07-26にも0.1〜0.2倍のゴミ値が45頭混入していた」と判明し、
2日限りの過去事象ではなく直近開催でも起きている継続的リスクと分かったため、
ユーザーの承認を得てバリデーションを実装した。

JRAの単勝オッズは最低1.0倍（元返し）で、**1.0未満は原理的に存在しない**。
オッズ昇順で`popularity`を導出しているため、0.1倍が1頭でも混入すると
**そのレース全体の人気順位が壊れ、残差学習モデルのbase_marginが誤る**。

#### 実装内容（2箇所：入口と安全網）
1. **`src/scraper/jra_scraper.py` `apply_odds_to_races()`（ゴミ値の入口）**：
   反映条件を `tansho > 0` → `tansho >= MIN_VALID_WIN_ODDS(=1.0)` に変更。
   弾いた頭数を警告ログに出す
2. **`src/features/engine.py` `_sanitize_win_odds()`（安全網）**：
   `calc_all()`のpopularity導出**直前**に呼び、1.0倍未満を`None`に落とす。
   スクレイパー以外の経路（history.db由来・Colab等）から入っても防げる。
   `None`にすることで既存の`or 999`フォールバックに乗り、人気順位では
   最後尾へ回る。市場確率・EV等も既存の欠損時フォールバックに乗るため
   追加の分岐は不要

同一レース・同一頭数の警告は1回だけ出す（`_INVALID_ODDS_WARNED`、
`_warn_xgb_inference_fallback`等と同じ設計）。

#### 実データでの効果検証（2026-07-26の壊れたlatest.jsonに適用）
| 項目 | 値 |
|---|---|
| 人気順位が修正されるレース | **16レース** |
| 順位が変わる馬 | **169頭** |
...
#### ⚠ この修正の限界（重要・過信しないこと）
**レース内の全馬が異常値または欠損の場合、正しい人気順位は復元できない。**
元データに情報が無いため、サニタイズ後は全馬`None`となり馬番順（任意）の
順位が振られる。実際に07-26の中京R1・R4はこのケースだった。

この修正が確実に防ぐのは以下：
- 0.1倍のゴミ値が「1番人気」という**嘘の情報**を作ること
- `market_prob = 1/0.1 = 10.0`（確率が1を超える不正値）がEV計算に流れること
- ゴミ値がそのままアプリに「0.1倍」と表示されること

**オッズ取得が全滅した場合の予想品質そのものは救えない**。そのケースは
data_quality（2026-07-25②導入）の`odds_coverage`低下としてアプリ上に
警告表示され、refresh実行（当日朝の再取得）で復旧するのが正しい対処。

#### 正常データへの影響：なし
1.0倍以上のオッズ・`None`・`0`（未取得）はいずれも一切変更しない。
テストで「正常データでは挙動が変わらないこと」を回帰テストとして固定した。
そのため**フォワード測定を汚さずに入れられる**（変更凍結の趣旨と矛盾しない）。

#### テスト
`tests/test_odds_validation.py` 新規9テスト。North Starに従い
**2026-07-26に実際に起きた形**（オッズ欠損と0.1倍台が同一レース内で同居）
を再現して検証：
- 1.0倍未満が`None`化され、1.0倍ちょうど・正常値・None・0は変更されないこと
- **0.1倍の馬が「1番人気」にならず、実オッズ2.4倍の馬が1番人気になること**
  （修正前のコードでは実際に失敗することを確認済み）
- ゴミ値そのものがクリアされ表示・EV計算に流れないこと
- スクレイパー側で1.0倍未満が反映されないこと／正常値は従来通り反映されること
- 同一レースの警告が1回だけであること

`python -m pytest tests/ -q` は414テスト通過（405+9、回帰なし）。

#### 未対応（残る根本課題）
`find_r01_odds()` のオッズ取得失敗**そのもの**は未修正
（2026-07-25④で診断ログを追加したのみ）。今回のバリデーションは
「壊れたデータが下流を汚すのを防ぐ」対症療法であり、取得失敗の再発は防げない。
次回発生時に診断ログから原因を特定すること。

---

### 2026-07-27⑦：⑥の「オッズ異常は2日のみ」という記録は誤りだった（07-26にも発生）

#### 訂正の経緯
⑥で「1.0倍未満のオッズ異常は2026-06-28と07-05の2日のみ、07-11以降は全て正常」と
記録したが、ユーザーから「**7/26にも発生した。ほとんどの馬がオッズなし、少しの馬で
0.1と0.2。その後（あなたに）改修してもらって再取得した**」という指摘を受けた。
実データで確認したところ**ユーザーの指摘が正しく、⑥の記録が誤り**だった。

#### 🔴 なぜ見落としたか（今後同じ調査をする時の必須知識）
`race_predictions` は同一 race_id への再実行で**上書きされる**（UNIQUE制約）。
そのため refresh 実行や再実行で修復されると、**DBには修復後の姿しか残らない**。
⑥ではDBだけを見て「07-11以降は全て正常（最小1.10倍）」と判断したが、これは
「障害が起きなかった」ではなく「**障害の痕跡が上書きで消えていた**」だった。

> 🔑 **オッズ取得障害を調査する時は、DBではなく `data/latest.json` の
> git履歴（`git show <commit>:data/latest.json`）を辿ること。**
> 各ワークフロー実行が生成した"その時点の姿"がコミットとして残っている。

#### 実際に起きていたこと（latest.jsonのgit履歴から復元）
2026-07-25（土）19:06 JST の週次実行が生成した**日曜7/26の予想**：

| 実行 | 時刻(JST) | commit | オッズなし | 1.0倍未満 |
|---|---|---|---|---|
| 土曜夜の週次実行 | 07/26 07:06 | `611fc34` | **183/420頭 (43.6%)** | **45頭**（0.1〜0.2倍） |
| refresh 1回目（手動） | 07/26 12:39 | `e74c9ba` | 1頭 (0.2%) | 0頭 |
| refresh 2回目（手動） | 07/26 13:53 | `bcf8185` | **0頭** | **0頭** |

3会場とも均等に被害（中京39%・新潟48%・札幌45%が欠損）。
原因は 2026-07-25④ で調査した `find_r01_odds()` の全滅
（新潟・中京・札幌の3会場すべてで「オッズR01: 未発見」）。

#### 🔴 判明：0.1/0.2倍は独立したバグではなく、オッズ取得失敗の副産物
レース単位で分類すると、**オッズなしと0.1倍台が同居しているレースが14レース**、
オッズなしのみ4レース、0.1倍台のみ2レース、正常14レース。両症状が強く相関して
おり、**オッズ取得が部分的に失敗した時に「欠損」と「ゴミ値」が同時に出る**構造。
⑥では別々の問題として扱っていたが、同一原因の2つの症状と見るのが正しい。

#### refreshモードの真の価値（⑥の評価は過小だった）
⑥では refresh の効果を「人気順位のズレが平均1.53→1.37位」と控えめに評価したが、
実際にはそれ以上に**オッズ取得障害からの復旧手段**として機能していた
（オッズ欠損43.6% → 0%）。この2回の refresh は、ユーザーからの
「まだレースは始まっていない、仕組みを変えれば間に合う」という指摘を受けて
当日中に refresh モード（PR #103）を実装し、**手動で起動したもの**。

#### ⚠ 自動実行はまだ一度も動いていない
`weekend.yml` の `cron: '0 23 * * 5,6'`（08:00 JST 土日）は自動実行の設定だが、
これを含む PR #103 のマージが **2026-07-26 21:23 JST**（日曜の全レース終了後）
だったため、**7/26時点では自動実行の設定自体が本番に存在しなかった**。
上記2回の refresh はすべて手動起動（workflow_dispatch）。
**初回の自動実行は次回開催の 2026-08-01(土)・08-02(日) 朝8時**になる。

#### 今後
- ⑥に記録した「オッズ1.0倍未満を弾くバリデーション」の必要性は、
  07-26にも発生していたことで**さらに高まった**（2日限りの過去事象ではなく、
  直近の開催でも起きている継続的リスク）。ただし変更凍結の合意により未実装
- 根本原因である `find_r01_odds()` の取得失敗自体は未修正
  （2026-07-25④で診断ログを追加したのみ）。次回発生時にログから原因を特定する

---

### 2026-07-27⑥：base_margin成熟度ズレを実測、補正案はボツと判定＋オッズ異常値を発見

#### 背景
「予測精度の根本改善はもう少しデータが揃ってからでいいか」という確認を受け、
2026-07-24⑤で記録した**base_margin成熟度ズレ**（学習時＝確定人気、推論時＝
発売直後の薄いオッズ由来人気）が本当に「データ待ち」なのかを検証した。

結論から言うと**この課題はデータ待ちではなかった**。必要なのは「レース数」では
なく「朝と直前のオッズのペア」で、`odds_snapshots` に既に255レース分あった。
そこで実測し、補正の設計可能性まで検証した。

#### 実測①：ズレは実在する（255レース・3,178頭）
| 指標 | 値 |
|---|---|
| 人気順位の平均絶対ズレ（朝→直前） | 1.50位 |
| 順位が変わらなかった馬 | 34.8% |
| ±1位以内 | 64.5% |
| 3位以上ズレた馬 | 19.5% |
| **1番人気が入れ替わったレース** | **34.9%（89/255）** |
| AI本命(RL1)自身の平均絶対ズレ | 1.05位 |

「3レースに1回は1番人気が入れ替わる」のは無視できない大きさ。

#### 実測②：ズレの構造は「平均回帰」
| 朝の人気帯 | 平均ズレ | 中央値 | 下がる% | 上がる% |
|---|---|---|---|---|
| 1番人気 | +0.93 | 0 | 34.9% | 0%（構造上不可） |
| 2-3 | +0.87 | 0 | 41.2% | 20.7% |
| 4-5 | +0.35 | 0 | 35.8% | 33.7% |
| 6-9 | -0.11 | 0 | 31.6% | 39.4% |
| 10+ | -0.90 | 0 | 22.0% | 44.8% |

上位人気は下がり、下位人気は上がる（平均回帰）。ただし**全帯で中央値0**、
つまり典型的な馬は動かず、一部だけが大きく動く。

#### 🔴 設計結論：補正係数案はボツ
人気帯別の平均シフトで補正した場合の改善を計算した結果：

```
朝人気をそのまま使う          : MSE 5.688 (RMSE 2.38位)
人気帯別の平均シフトで補正    : MSE 5.237 (RMSE 2.29位)
→ 改善わずか 7.9%
```

中央値が全帯で0であることが示す通り、**ズレの大半は帯情報から予測できない
特異的なもの**であり、補正係数を作っても効果がほぼ無い。この設計方針は
採用しない。

#### 代わりに有効だったもの：refreshモード（7/25導入済み）
「古い人気を補正する」のではなく「新しい人気を使う」方が直接的。
7/25に導入した当日朝refreshの前後を比較すると実際に改善していた：

| 期間 | R数 | 平均絶対ズレ | 順位不変率 |
|---|---|---|---|
| 導入前（〜7/19） | 188 | 1.53位 | 32.9% |
| 導入後（7/25〜） | 66 | **1.37位** | **40.8%** |

改善幅は小さいが方向は正しい。さらに詰めるならモデル変更ではなく
**refresh実行を発走に近づける運用変更**（現在は08:00 JST固定）が筋。
ただしN=66と少なくまだ断定はできない。

#### 🔴 副次発見：朝オッズに1.0倍未満の異常値が43頭
検証中、オッズ変化率の平均が+407.9%（中央値+10.5%）と極端に乖離していたため
外れ値を調べたところ、`race_predictions.tansho_odds` に**単勝で存在し得ない
1.0倍未満の値**が入っていた。
...
> ⚠ **【2026-07-27⑦で訂正】以下の「2日のみ／07-11以降は正常」は誤り。**
> `race_predictions` はrefresh実行で上書きされるため、DBには最終状態しか
> 残らない。実際には **2026-07-26にも大規模に発生していた**（後述の
> 「2026-07-27⑦」セクション参照）。この種の障害を調査する時は
> **DBではなく `data/latest.json` のgit履歴を辿ること**。

- 発生日は **2026-06-28（5頭）と2026-07-05（38頭）の2日のみ**。
  07-11以降の5開催日は全て正常（最小1.10倍）
  → ⚠ 誤り。07-26も発生（訂正セクション参照）
- `odds_snapshots`（GAS経由の直前オッズ）側は5,553行すべて正常
- 大半は取消馬ではなく**実際に出走している**（着順あり）
- 影響は**15レース（全332レースの4.5%）**。1頭でも異常値があるとオッズ昇順で
  導出する`popularity`が壊れ、**そのレース全体のbase_marginが誤る**
  （例: 実際は人気薄の馬が0.1倍で「1番人気」扱いになる）

`calc_all()`のpopularity導出には下限バリデーションが無く、
再発すれば静かにレース全体を汚す。

#### 今後の方針（ユーザーと合意）
直近で買い目ロジックを大きく変えた（07-27③④⑤）ため、**2〜3週間は変更を
止めてフォワードデータを取る**。理由は精度の問題ではなく測定の綺麗さのため。
- 単勝専用モデル（rank:ndcg残差版）は学習自体は今すぐ可能だが、
  「良くなった」の判定にフォワードデータが要る（現状137レース、目安N=300）
- base_margin補正は上記の通りボツ。refresh時刻の見直しは別途検討

#### 変更範囲
本セッションは**分析・記録のみでコード変更なし**。
提案として「オッズ1.0倍未満を弾くバリデーション」があるが、
変更凍結の合意に従い実装せず記録に留めた（正常データでは一切挙動が
変わらないため、フォワード測定を汚さずに入れられる性質のものではある）。

---

### 2026-07-27⑤：bet_simulationの決済処理を実装＋馬単の的中判定バグ修正

④の「次にやる予定」の2番・3番を実施した。

#### 🔴 発見①：bet_simulation に決済処理が存在せず、2,160行が永久に未決済だった
`log_bet_simulation()` は全6券種を「買った想定」で毎レース記録するが、
コード全体を検索しても **`UPDATE bet_simulation` が一箇所も存在しない**。
そのため記録された2,160行のうち96〜100%が `is_hit=-1`（未決済）のまま放置され、
**券種別の有効性を評価するデータが一切蓄積されていなかった**。

| 券種 | 未決済率 |
|---|---|
| ワイド・三連複・馬単・馬連 | **100%** (360/360) |
| 単勝・複勝 | 96% (346/360) |

実際に買った `bets` は452点しかないのに対し、`bet_simulation` は2,160点あり、
決済すれば**5倍近いサンプル**が得られる。「馬連や三連複が有効か分からない」
状態だったのは、単にデータを採点していなかったため。

#### 対応①：`settle_bet_simulation()` を新設
`src/utils/db.py` に追加し、`scripts/weekend.py` の `fetch_and_save_results()`
（土日両方が通る共通経路）から呼ぶようにした。

**設計上の重要な判断**：payout は `odds_est`（記録時の推定オッズ）ではなく
**実際の配当**（`dividends`）を使う。推定オッズで決済すると「推定が甘い券種ほど
成績が良く見える」というバイアスが入り、券種比較の目的を壊すため。
実配当が取れない場合は `is_hit` のみ更新し `payout=0` に留める
（回収率の分母には入るが分子には入らないため過大評価にならない）。

`is_hit=-1` の行のみを対象とするため再実行しても二重計上されない
（冪等性はテストで固定）。決済失敗時も結果取得自体は止めない。

#### 🔴 発見②：馬単の的中判定が着順を見ていなかった
`check_and_update_bets()`（`db.py`）の判定が馬連・馬単で共通の
`h1 in top3[:2] and h2 in top3[:2]` になっており、**馬単なのに着順を
検証していなかった**。3→8で買って実際が8→3でも的中扱いになり、
さらにその際 `umatan` の実配当（＝正しい順序の配当）を払い戻していた。

現在馬単は使っていない（最終記録2026-07-04）ため実害は限定的だが、
過去の集計を汚す。またこの関数には**テストが1件も存在しなかった**。

#### 対応②：着順判定に修正＋回帰テスト新設
馬連は `h1 in top2 and h2 in top2`（順不同）、馬単は `[h1, h2] == top2`
（着順一致）に分離。修正前のコードで実際に失敗することを確認済み。

#### テスト
`tests/test_settle_bet_simulation.py` 新規16テスト。North Starに従い、
**手打ちINSERTではなく実際に `log_bet_simulation()` で書き込んだ行**を
実際の sqlite3 から読み出して決済する形で検証：
- 6券種すべてが決済され `is_hit=-1` が残らないこと
- payout が推定オッズではなく実配当であること
- **馬単は着順が逆なら不的中／馬連は順不同で的中**（両者の差を固定）
- 外れが `is_hit=0` で確定すること（-1のまま残さない）
- 配当が取れない時は的中でも `payout=0` に留めること
- 結果が無いレースは触らない／再実行で二重計上しない（冪等性）
- `check_and_update_bets` の馬単着順判定3件（修正前は実際に失敗することを確認済み）

`python -m pytest tests/ -q` は405テスト通過（389+16、回帰なし）。

#### 次回結果取得時に起きること
次回「土曜結果」「日曜結果」ボタン押下で、**蓄積済み2,160行が一括で決済される**
（過去のレース結果が `all_results` に含まれる分のみ。含まれない過去分は
未決済のまま残る）。ワークフローのログに券種別の的中数・ROIが出力される。
ここで初めて「馬連・三連複・ワイドが有効か」の判断材料が得られる。

---

### 2026-07-27④：単勝をboost限定化＋複勝にboost優先を導入

#### 背景
③のフィルタ導入後、「他に回収率を上げる提案は？」という要望を受け、
`media.githubusercontent.com` 経由で取得した本番keiba.dbを実データ分析した。

#### 🔴 発見：現状の買い目は儲かるセルの馬をほぼ買えていなかった
実際に打った単勝のうち`race_predictions`と突合できた34点を判定別に見ると：
...
従来ロジックが単勝候補をRL上位3頭に限定していたため、実測で最も良いboostセル
（市場中位人気×RL4-5）の馬はほぼ一度も買われていなかった。

#### 提案1（実装）：単勝はboost馬が居る時だけ買う
③ではboost馬を候補に追加したが、boost馬が居ないレースでは従来通りRL上位3頭から
買っていた。実データではこれが負け筋と判明：

| 買い方 | 対象 | ROI |
|---|---|---|
| boost馬を1点 | 229レース | **164.5%** |
| 従来RL上位3頭（boost馬が居るレース） | 611頭 | 93.9% |
| 従来RL上位3頭（boost馬ゼロのレース） | 199頭 | 94.8% |

**out-of-sample検証（学習期〜7/19でセル判定→検証期7/20〜の68レースで評価）**：

| 戦略 | 購入 | 的中 | 勝率 | ROI |
|---|---|---|---|---|
| A. 従来（RL上位3頭） | 68 | 12 | 17.6% | 74.3% |
| B. ③の実装（boost優先＋従来併用） | 68 | 18 | 26.5% | 193.8% |
| **C. boost限定（本実装）** | 46 | 12 | 26.1% | **241.1%** |

`_select_win()` を、マトリクスが有効な場合はboost馬のみを候補とし、
boost馬が居なければ単勝を見送るよう変更した。

> ⚠ **調べた上でボツにした案**：「boost馬ゼロのレースは丸ごと見送る」も検討したが、
> そのレースの従来ピックは94.8%で、boost馬が居るレース(93.9%)と**変わらなかった**。
> 「悪いレース」が存在するのではなく「良い馬」が存在するだけなので、
> レース単位の足切りには意味がないと判断した（馬単位の限定のみ実施）。

#### 提案2（実装）：複勝にboost優先を導入
③では「複勝回収率は未測定」としてboost拡張を見送っていたが、`results` テーブルの
実払戻（`fukusho_payout`）で測定できることが判明したため検証した：

| 判定 | N | 複勝率 | 複勝回収率 |
|---|---|---|---|
| boost | 387 | 34.6% | **97.2%** |
| neutral | 2,996 | 22.3% | 67.1% |
| suppress | 765 | 19.9% | 77.8% |

単勝と違い100%には届かないため**boost限定にはせず、優先順位のみ変更**
（買い目点数は維持しつつ期待値の高い馬から埋める）。

##### ⚠ 測定時に踏んだ罠（今後同じ分析をする時の注意）
`results` テーブルは**6着までしか保存されていない**（7着以降は各23件のみ）。
最初に `race_predictions` と INNER JOIN したところ7着以下の馬が全部落ち、
複勝率が約50%・全判定でROI 130%超という**明らかにおかしい結果**になった。
LEFT JOIN で全出走馬を残し、`actual_place<=3` で的中判定・払戻は3着以内の
馬だけ引く形に直して正しい値を得た。同種の分析では必ずJOINで母集団が
削れていないか確認すること。

#### 安全設計
- `is_matrix_active(base_dir)` を新設。マトリクスが未生成の環境（初回実行・
  旧latest.json等）では**boost限定を適用せず従来挙動**を保つ
  （単勝が一切買えなくなる事故を防ぐ）
- 環境変数 `RANK_MATRIX_FILTER=0` でも従来挙動に戻る
- `base_dir` 省略時（旧呼び出し）も従来挙動

#### テスト
`tests/test_rank_matrix_filter.py` に6テスト追加（計18）：
- boost馬が居ない時に単勝を見送ること／居れば選ぶこと
- マトリクス不在時・kill switch時に従来通り買うこと（事故防止の回帰テスト）
- 複勝でboost馬がRL順位を追い越して優先されること／マトリクス不在時はRL順のまま
- `index.html` 側もJS/Python間で同一入力→同一判断になることを `node -e` で確認
- `python -m pytest tests/ -q` は389テスト通過（383+6、回帰なし）

#### 次にやる予定（ユーザー合意済みの順序）
1. ~~単勝boost限定＋複勝boost優先~~（本セッションで完了）
2. **`bet_simulation` の決済処理を実装**：2,160行が96〜100%未決済のまま
   （コード中に `UPDATE bet_simulation` が**存在しない**）。実装すれば実際に
   買った452点だけでなく2,160点分の全券種データが学習材料になる
3. **馬単の的中判定バグ修正**：`db.py:614` の判定が
   `h1 in top3[:2] and h2 in top3[:2]` で**着順を見ていない**。馬単は1着2着の
   順序が合って初めて的中だが、逆順でも的中扱いになる（現在馬単は未使用のため
   実害は限定的だが集計を汚す）
