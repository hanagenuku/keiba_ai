...
### 最終更新: 2026-08-08③（🎯 Plackett-Luceレーティング導入。AUC +0.037 / 回収率 +40pt。初めて両方が動いた）

#### 経緯：伸びしろを「測って」から手を打った
...
#### 何を作ったか
Plackett-Luce モデル。1レースの着順**全体**を1つの観測として扱う:

    P(着順 π) = Π_k  exp(θ_π(k)) / Σ_{j>=k} exp(θ_π(j))

「Aに勝ったBに勝ったC」の連鎖から、**直接対戦していない馬同士も比較できる
1つの尺度**に載せる。既存の `f_rl` / `f_speed_fig_avg` は過去走スコアの
重み付き平均で、**強い相手への2着と弱い相手への2着が同じ扱い**だった。

#### 効果（3シード平均・本番構成＝市場アンカー＋残差学習）
...
**PLレーティング5個だけで AUC 0.7918** — 既存126特徴量(0.7697)を単独で上回る。

⚠ **まだ「勝てる」ではない**: 3本抜き90.7%、窓Aは87.8%、10日・349レース。
North Star #7 の水準には届いていない。**AUCが上がっても回収率が上がる保証は
無い**が、今回は珍しく両方が同じ方向に動いた。

#### 🔴 リークでないことの検証（North Star #8）
**プラセボ対照が決め手。** レース内でθを馬にランダムに振り直し、
...
**5窓すべてでフラット**（構造的リークがあればプラセボでも改善が出る）。
加えて: 初出走馬23,699件のθがすべて厳密に0／同じ馬が同じ日に複数走っても
θが変化した件数0／θと着順の相関 -0.357。

#### 🔑 学習/推論パリティの担保（このプロジェクト最大の事故源）
**読み取りコードを1本にした。** 学習も推論も
`calc_features_for_xgb` → `engine._PL_RATINGS` を読む、で同一。
違うのは**書き込み側だけ**:
- 推論: `init_engine` が `data/pl_rating.pkl` をロード
- 学習: `build_training_data` が日付順に走りながら `advance_day()` で再現

**実測で完全一致を確認**: θの最大差 0.0000000000 ／ 出走回数の最大差 0 ／
実レース18頭のフィールド特徴量も完全一致。

⚠ **日単位で処理する**のが要。その日の**全レースの特徴量を作り終えてから**
結果でθを更新する（本番は朝に全レースを予想するため）。
...
| `sunday-results.yml` | 週次更新を追加（`|| true` で非ブロッキング） |

#### テスト
`tests/test_pl_rating.py` 新規15テスト。特に固定したもの:
- **同じ日の1R目の結果が2R目の特徴量に漏れないこと**（リーク防止の核心）
- **強い相手を破ると評価が上がる／強い相手への敗戦は罰が軽い**
  （既存 f_rl との本質的な差。ここが壊れたら入れる意味が無い）
- 保存→ロードで完全一致すること（パリティの土台）
- pkl未生成でも例外を出さず学習時と同じ既定値になること

⚠ **テストの作り方で一度失敗した**: 「無名馬を10連勝させて弱い相手を作る」と
書いたら、その無名馬が強豪になってしまい両者が同値になった。
θを直接指定する形に直した。

#### ⚠ 効かせるには再学習が必要
`xgb_feature_cols.json` は129特徴量でPLの5個を含まない。
`xrow = {c: xfeats.get(c, 5.0) for c in _XGB_FEATURE_COLS}` の仕組みにより
**新しい列は静かに無視される**（推論は壊れないが効果もゼロ）。
`build_training_data()` → `train_xgb()` の再実行が必要。
