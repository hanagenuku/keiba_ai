...
## 🚩 次セッションはここから読む（2026-07-27終了時点のサマリ）

### いま何が動いているか
**買い目が「人気×RL乖離マトリクス」の実績で選ばれるようになった**のが最大の変更。
単勝は**boost馬に限定して全頭買う**（boost馬が居ないレースは見送り）。
現在boostされるセルは「市場6-9人気×RL4-5」「市場2-3人気×RL4-5」の2つで、
**AIの本命(RL1)ではなく中位評価の馬**が対象という構造。

### ⚠ 最重要の注意：まだ確証は無い
ウォークフォワード検証で通算ROI 182.8%だが、**評価できたのは4開催日だけで
うち2日がマイナス**（7/11は16点買って0的中）。プラスの大半は7/25・7/26の2日。
**確証には10〜15開催日（目安N=300点）が必要**で、あと6〜8週かかる。
ユーザーは現在**エア馬券で運用中**。

過去に「95%信頼区間124〜405%」と報告したが、これは2日に固まったデータを
独立サンプルとして扱った**過信であり撤回済み**。同様の統計を出す時は
必ず「何開催日ぶんか」を確認すること。

### 次の週末（2026-08-01・08-02）で確認すること
今日は変更を多数入れたので、**次回の実行結果確認が最優先**:

1. **refreshの自動実行が初めて動く**（cron 08:00 JST 土日。設定は入っているが未稼働）
2. **prediction_snapshots に initial と refresh の両方が入るか**
3. **bet_simulation 2,160行が一括決済される** → 馬連・三連複・ワイドの有効性が初めて判定可能に
4. **単勝の買い目が全boost馬になっているか**（💎バッジの馬が全部入るはず）
5. **オッズ異常の警告ログが出ないか**（出なければ正常）

### 🔑 調査時に必ず使う2つの手法（今日これで事故を防いだ/見つけた）
...
2. **オッズ取得障害はDBではなく `data/latest.json` のgit履歴を見る**
   ...
   これが原因で私は「07-26に障害は起きていない」と誤記録した）

### 変更凍結の合意
直近で買い目ロジックを大きく変えたため、**2〜3週間はモデル変更を止めて
フォワードデータを取る**方針でユーザーと合意済み。理由は精度ではなく
測定の綺麗さ。ただし「正常データで挙動が変わらない」修正
（バリデーション・記録基盤）はこの制約の対象外として実施した。
