...
### 次にやること（優先順）
1. **Driveの土日ノート・金曜ノート**のセル5（特徴量エンジン）を手動差し替え
   - 「# 特徴量エンジン v5 クリーン版」から始まる長いセルを丸ごと差し替え
2. **週末の実運用**で動作確認（強制アップデートセル → 特徴量エンジンimport → 予想生成）
3. 将来: 実行時刻によるレース自動選別機能の実装検討

### 未解決・保留中
- 土日・金曜ノート（Drive版）のセル5はまだ旧インライン定義のまま（急ぎでなければ不要）
