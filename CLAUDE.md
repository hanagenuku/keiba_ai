...
### 再評価結果：Val AUC 0.7621 → **0.8013**（+0.0392）

| | 復旧前 | 復旧後 |
|---|---|---|
| 検証窓のアンカー欠損 | 58.0% | **0.0%** |
| Val AUC | 0.7621 | **0.8013** |
| Val Brier | 0.1786 | **0.1776** |

⚠ この差には学習データ増（Driveマージで3.5年・151,938行）も混ざるが、
**学習データ量の寄与は別途測定で+0.0036しかない**（1年学習0.7874 vs 3年学習0.7910）
ため、**大半（約+0.035）はアンカー復旧の効果**。
...
⚠ **未解決の設計課題**: 学習時＝確定オッズ、推論時＝朝の薄いオッズという
非対称（2026-07-24⑤の「深刻度：高」課題そのもの）。しかも**オッズの絶対値は
順位よりブレが大きい**ため、順位ベースの現行アンカーより悪化する危険もある。
現状 `_apply_popularity_drift()` は順位のドリフトしか注入していない。
着手する場合は `odds_snapshots`（朝と直前のペア）で実測してから設計すること。

⚠ **期待値の正直な見積り**: +0.0147 AUCが控除率20%を超える保証はない
（North Star #9）。採用判断はAUCではなく**回収率**で行うこと。
