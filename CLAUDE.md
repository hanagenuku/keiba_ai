...
> 📌 **このファイルが唯一の情報源。** 現状・経緯・既知の制限はすべてここにある。
> `docs/KEIBA-AI_引き継ぎ書_追補_2026-06-28.md` は 2026-08-20 に薄いポインタへ畳んだ
> （残っているのは「新しいセッションの始め方」だけ）。同じ内容を2箇所に置くと
> 片方だけ更新されるため、**現状をあちらに書き足さないこと**。
> なお `docs/history_db_schema.md`（history.dbのカラム定義・充足率）は現役。
...
| 2026-08-20② | 軸の確度の較正を直せばレース選択の物差しになるのでは → **較正ではなく解像度の問題だった**。RL1だけを取り出すと fuku_prob の AUC は **0.548**（95%CI [0.502, 0.597]、全馬なら0.723）。再較正は単調変換なので順序の情報を作れず、正しく較正すると「どの確度でも約0.53」になるだけ。softmax温度を3.5〜12まで振ってもRL1のECEは0.09を下回らない | **直す対象ではないと確定**。`MIN_AXIS_FUKU_PROB=0.55` は現状維持（根拠は較正ではなく2026-08-08②のROI差）。副産物としてアプリの**「AI xx%」バッジがAUC 0.483＝情報なし**（正体は「本命がどれだけ人気薄か」・人気順位との相関+0.382）と判明し削除した |
...
#### ✅ A-4：引き継ぎ書（追補）を薄いポインタに畳んだ（二重管理をやめた）
