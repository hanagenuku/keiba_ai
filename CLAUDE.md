...
### 最終更新: 2026-07-10 セッション（EV買い目のRL上位ベース再設計）
...
#### EV買い目のRL上位ベース再設計（ブランチ: `claude/betting-strategy-investigation-5k83a0`）

**問題**: 旧設計は EV = prob × odds を最大化 → Gumbel T=2.5 で確率がフラット化し、
EVがオッズに単調増加 → 常に最低人気を推す構造的欠陥（9頭立て9番人気の単勝推奨等）。

**新設計**: 買い目の起点を「EV最大」→「RL上位の中から選択」に変更。
| 券種 | 旧 | 新 |
|------|-----|-----|
| 単勝 | EV最大の1点 | RL上位3頭からオッズ妙味(2〜30倍)のある馬1点 |
| 複勝 | EV基準で確率上位5頭から2点 | RL上位5頭からRL順で2点 |
| 馬連 | EV上位5点 | RL上位5頭の組み合わせ(RL3含む)で5点 |
| 三連複 | 変更なし | 変更なし（軸構造ベース維持） |
| EV閾値 | 1.15〜1.30（券種別） | 1.0（足切りのみ） |

- `src/betting/bet_optimizer.py`: `_select_win/place/quinella` を RL ベースに書き換え
- `_build_rl_ranking()` 新設（horses の rl_rank を {num: rank} マップに変換）
- `tests/test_bet_optimizer.py`: RL選択テスト4件追加（23テスト全通過）
- 全137テスト通過
