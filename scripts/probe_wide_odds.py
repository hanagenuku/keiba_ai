...
⚠ リクエスト数: find_r01_odds の suffix 探索が最大256件、券種スキャンが26件。
   1レースぶんのみで、券種スキャンには1秒の待機を入れる。

🔴 **開催日（土日）にしか実行できない。**
   オッズ盤は発走前にしか存在せず、開催情報を引く出走表一覧
   (pw01dli00) もこれから開催されるレースしか載せない。
   月曜に走らせると「開催情報が見つかりません」で必ず空振りする
   （2026-08-17に実測。前日の日曜は同じ日付で取得できていた）。
"""
import re
import sys
import time
