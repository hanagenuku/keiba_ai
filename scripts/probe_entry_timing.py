...
## 第3回(2026-09-10 木 10:05 JST) — 木曜の朝も出ていない。しかも先週末が消えた

■ 今週末(20260912/20260913) は出馬表・オッズ・レース結果・払戻金のどこにも
  **まだ1件も無い**。木曜の朝は境界ではない。

■ 🔴 予想外だったこと: **先週末(0905/0906)の6件も消えていた**。
  出馬表一覧 `pw01dli00/F3` の CNAME リンクは12件すべてグローバルナビで、
  `pw01drl*` が1件も無い（水曜は pw01drl10×6 があった）。
  ページ内の8桁日付も **0件**。つまり今の出馬表一覧は**空の状態**。
      水曜: 先週末6件（pw01drl10 = 過去分）
      木曜: **0件**
      金曜以降: 今週末が入るはず（未測定）
  過去分を落としてから今週分を入れる、という入れ替えの谷間にいる。

■ ⚠ 前回の推論を1つ訂正する。「JRA は一度載せたものを引っ込めないので
  火曜にも無かったはず」と書いたが、**引っ込めることが実測で分かった**（上記）。
  火曜について言えるのは「レース前の出馬表を載せてから消すことはしない」
  という別の（弱い）根拠だけ。実測ではない点は変わらない。

■ 特別レース登録馬(accessT pw03trl00/29) は水曜と同じ12レースのまま。
  中身も `馬名 | 負担重量` の2列だけで変化なし。
  → 木曜の時点でも、今週末について機械で取れるのは
    「特別レース6鞍×2日の登録馬名と斤量」だけ。

■ 残る未測定: **金曜**。次の計測でここが埋まる。

## 第4回(2026-09-11 金 10:05 JST) — ✅ 境界は金曜。これで決着

■ 今週末(20260912/20260913)が出馬表一覧 `pw01dli00/F3` に**出た**。
  昨日の同時刻は 0件だったので、境界は **木10:05 JST 〜 金10:05 JST の間**。

      4回中山3日 馬番確定   accessD.html  pw01drl00062026040320260912/E4
      4回阪神3日            accessD.html  pw01drl00092026040320260912/C2
      4回中山4日            accessD.html  pw01drl00062026040420260913/D2
      4回阪神4日            accessD.html  pw01drl00092026040420260913/B0
      9月12日(土曜)・9月13日(日曜)          pw01d5010A0/B8

■ 🔑 4つのうち**中山土曜だけが「馬番確定」**で、残る3つはラベルが無い。
  出馬表が載ることと馬番が確定していることは**別**で、同じ金曜の朝でも
  開催ごとに進み方が違う。1つの日付で「出た/出ない」を判定すると取り違える。

■ オッズ盤は**まだ無い**。一覧 `pw15oli00/6D` へのリンクはグローバルナビの
  常設リンクで、今週末の `pw15ode*` は1件も現れていない。
  → D' のワイド盤プローブを金曜朝に前倒しする意味は無い。
    土日 08:10 JST（現行の cron）のままでよい。

■ 特別レース登録馬は水曜・木曜と同じ12レース・`馬名|負担重量` の2列で変化なし。
  金曜に出馬表が出ても、こちらは更新されない別系統。

## この調査の結論（3日ぶんの実測）

      火曜  未測定（推論のみ・根拠は弱い）
      水曜  今週末 0件。載っているのは先週末の過去分6件だけ
      木曜  **0件**。過去分も落ちて一覧が空になる谷間
      金曜  ✅ 今週末4件が出る。ただし馬番確定は一部の開催だけ
      金曜夜 本番(friday-predict.yml)が動く ＝ 実運用と整合

🔑 本番の「金曜夜に翌日を取る」設計は、JRA の掲載タイミングに対して
  正しい。金曜より前に前倒ししても取るものが無い。
⚠ 「載る日」は分かったが「金曜の何時か」は分かっていない。必要になったら
  金曜を数時間おきに測ること（今は必要が無いので測らない）。
