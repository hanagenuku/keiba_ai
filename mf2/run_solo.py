...
TASKS = [(g, w, s) for g in sorted(GROUPS, key=lambda k: -len(GROUPS[k]))
...
def job(t):
    ...
    if grp is not None:
        keep[:] = False; keep[GROUPS[grp]] = True
    win = D['windows'][w]
    sub = dict(win)
    for k in ('Xf', 'Xi', 'Xv'):
        sub[k] = win[k][:, keep]
    r = fit_one(sub, CID82, seed=seed)
    return dict(only=grp or '(なし)', win=w, seed=seed, n_feat=int(keep.sum()),
...
if __name__ == '__main__':
    ...
    df.to_csv(f'{BASE}/solo_raw.csv', index=False)
    print('saved solo_raw.csv')
