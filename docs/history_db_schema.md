...
| pace_label | TEXT | ペース分類ラベル（S/M/H） | 2026-07-21時点で長期未取得だった原因を特定・修正済み（`_extract_weather_pace`が「ペース」直後のH/M/S1文字のみ探索しており、実機の「ペース判定：ミドルペース」のような単語表記と不一致だった）。次回実行分から充足率改善見込み。`train_pace_model.py`の`_classify_pace()`はこのラベルが無くても`first_3f`から独自分類するフォールバックを持つため、影響は限定的だった |
| corner_pass_3 | TEXT | 3コーナー通過順位（同着グルーピング表記込みの生テキスト、例:`(1,*5)6,10(2,9)-(3,4)8=7`） | 2026-07-21〜。特徴量化は未実施、生テキストの収集のみ |
| corner_pass_4 | TEXT | 4コーナー通過順位（同上） | 同上 |
