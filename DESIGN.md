...
| HTMLの列位置（`tx[13]`等のインデックス）を決め打ちでパースする | JRA側のページ構造がわずかに変わるだけで別列を読んでしまい、しかも例外にならず気づかれにくい。可能な限り正規表現でのパターンマッチや`<a>`タグの有無等、位置に依存しない判定を使う（Stage3再スクレイプでbracket/win_odds/body_weightの列が実際にズレ、埋まり率0〜6.5%のまま長期間気づかれなかった） |
