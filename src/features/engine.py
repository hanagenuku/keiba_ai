...
def calc_all(race, bias_data=None):
    ...
    # 🔴 2026-09-01: 既定OFF。2,318レースのout-of-sample検証で機序が否定された。
    #   ① 補正の前提「この条件でAIは過小評価している」が成り立たない。
    #      補正対象 予測23.0%/実測22.4%（ズレ-0.7pt）に対し
    #      補正対象外 予測22.3%/実測21.7%（ズレ-0.7pt）で**完全に同じ**。
    #      条件固有の較正ズレは存在せず、5条件すべて実測が予測を下回っていた
    #      （＝引き上げる根拠がない）。
    #   ② 見かけの改善は偶然の温度調整だった。補正が当たる340レースでの
    #      単勝log-loss は -0.0094 改善するが、**当たらない1,978レースに
    #      同じ倍率を掛けても -0.0110** で同じだけ改善する。
    #   ③ 8条件のうち3つは factor=1.0・個別補正なしで literally 何もしない。
    #      札幌_芝_mile_良 の `position_bias` は get_correction_factor に
    #      ハンドラが無く、順位を動かさない（style_miss / class_miss も同様）。
    #   ④ 潜在バグ: 条件キーは track_condition を含むが、推論時は出馬表に
    #      馬場が無いため常に「良」（2026-08-27 W1）。良以外のレース（2026年で
    #      23.6%）は蓄積時に別キーへ入り、**推論時は永久に参照されない**。
    #   2026-07-28 の rank_matrix_filter と同じ判断（機序が否定されたので
    #   既定OFF・タグの蓄積と測定は残す）。ERROR_TAG_CORRECTION=1 で復帰。
    #   ⚠ 温度そのものには改善余地がある（大域倍率1.30で log-loss -0.0267）が、
    #      Harville側の fuku_prob は逆に過信している（2026-08-20②）ので
    #      単純には触れない。別テーマとして開いたままにする。
    if _BASE_DIR and os.environ.get('ERROR_TAG_CORRECTION', '0') == '1':
        try:
