...
# Gumbel-Max により P(勝利) = softmax(rating / T) と数学的に等価。
#
# 【履歴】2026-07-06 に T=2.5 を採用した。当時の実測は「Gumbel RL1平均35% vs
# 実勝率16%」で、過信を平坦化する方向の補正だった。
#
# 🔴 2026-08-20 に T=1.0 へ変更。理由は2つ重なっている:
#   ① 2026-07-26 に2重sigmoidバグを直し、rating が本来の（広い）log-odds幅に
#      戻った。T=2.5 は**潰れていた頃のスケール**で最適化された値だった
#   ② 残差学習モデルでは RL1 がほぼ市場上位人気に収束するため、RL1 の実勝率
#      自体が 16% → 37% に上がった。母集団も変わっている
#
# 【再検証】latest.json の全世代から rating を復元（rating = ability_margin +
# base_margin。calibrator(sigmoid(復元rating)) が保存 cal_prob と中央値
# 0.000038 で一致することを検算済み）し、206レース・6開催日 × 実着順で測った:
#
#   T      win log-loss   最上位馬の予測勝率   実勝率     勝率ECE   3着内ECE
#   1.0        1.9970           42.8%         36.8%     0.0077    0.0420
#   1.25       1.9972           34.1%         36.8%     0.0096      —
#   2.5        2.1634           18.4%         36.8%     0.0356    0.0602
#
# T=2.5 は**平坦化しすぎ**で、最上位馬を 18.4% としか見ていないのに実際は
# 36.8% 勝っていた（-18pt の過小評価）。6開催日すべてで T=1.0〜1.5 が最適、
# 日ブロックbootstrap 2,000回すべてで T=1.25 が T=2.5 に勝った。
#
# T=1.0 と T=1.25 は log-loss ではほぼ同値（データで区別できない）。
# **説明のつかない定数を置かない**方を採り、温度スケーリング自体をやめた。
#
# ⚠ これは確率表示を正直にする変更であって、エッジを作る変更ではない
#   （North Star #9）。回収率が上がる根拠にはしないこと。
# ⚠ 残る過信は別テーマ: 3着内確率の最上位帯は T=1.0 でも 0.640 予測 /
#   0.541 実測。2026-08-20 に見つけた「軸の確度が0.65で頭打ち」と同じ現象。
DEFAULT_GUMBEL_RATING_T = 1.0
